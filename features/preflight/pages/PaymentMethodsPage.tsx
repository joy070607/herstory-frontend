  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-neutral-50 px-4 py-3.5">
      ...
      <button
        ...
        onClick={onDelete}
        disabled={deleting}
        className="shrink-0 text-xs font-medium text-red-500 underline disabled:opacity-40"
      >
        삭제
      </button>
    </div>
...
export function PaymentMethodsPage() {
  ...
  const { data: cards, isLoading } = usePaymentMethods(memberId);
  ...
  const handleDelete = (cardId: number) => {
    ...
    deleteCardMutation.mutate(cardId, { onSettled: () => setDeletingId(null) });
  };
          ...
          <div className="flex flex-col gap-3">
            {cards.map((card) => (
              <CardRow
                key={card.cardId}
                card={card}
                onDelete={() => handleDelete(card.cardId)}
